import { sitePresetsRepo, type SiteSettingsSnapshot } from "../repositories/cms.sitePresets.repo";
...
class SitePresetApplyService {
  ...
  async activate(presetId: string, adminEmail?: string, notes?: string): Promise<ActivateResult | { error: string }> {
    ...
    if (seedMode === "createFromTemplate" && config.homepageTemplateId) {
      const newPage = await cmsRepository.create({
      ...
      const existingHome = await cmsRepository.findHome();
      if (existingHome) {
        await cmsRepository.update(existingHome.id, {
          template: config.homepageTemplateId,
        });
