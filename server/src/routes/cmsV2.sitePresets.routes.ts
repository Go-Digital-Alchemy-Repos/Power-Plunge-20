...
router.post("/", async (req, res) => {
  try {
    const result = await sitePresetsService.create(req.body);
...
router.put("/:id", async (req, res) => {
  try {
    const result = await sitePresetsService.update(req.params.id, req.body);
    if (result.error) {
...
router.delete("/:id", async (req, res) => {
  try {
    const result = await sitePresetsService.remove(req.params.id);
...
router.post("/:id/activate", async (req, res) => {
  try {
    ...
    const result = await sitePresetApplyService.activate(req.params.id, adminEmail);
    if ("error" in result) return res.status(404).json({ error: result.error });
    res.json(result);
  } catch {
