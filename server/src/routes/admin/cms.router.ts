...
router.get("/pages/shop", async (_req, res) => {
  const page = await cmsService.getShopPage();
...
router.get("/pages/check-slug", async (req, res) => {
  ...
  const result = await cmsService.checkSlug(slug);
  res.json(result);
});
...
router.get("/pages", async (_req, res) => {
  const list = await cmsService.listPages();
  res.json(list);
});
  ...
  try {
    if (req.body.contentJson !== undefined && req.body.contentJson !== null) {
      ...
      if (result.warnings.length > 0) {
        result.warnings.forEach((w) => console.warn("[CMS]", w));
      }
    }
...
router.put("/pages/:id", async (req, res) => {
  try {
    ...
    const page = await cmsService.updatePage(req.params.id, partial);
    if (!page) return res.status(404).json({ error: "Page not found" });
    res.json(page);
  } catch (err: any) {
    if (err.name === "ZodError") {
...
router.post("/pages/:id/set-home", async (req, res) => {
  const page = await cmsService.setHomePage(req.params.id);
  if (!page) return res.status(404).json({ error: "Page not found" });
  res.json(page);
});

router.post("/pages/:id/set-shop", async (req, res) => {
  const page = await cmsService.setShopPage(req.params.id);
  if (!page) return res.status(404).json({ error: "Page not found" });
  res.json(page);
});
...
router.post("/pages/:id/migrate-to-blocks", async (req, res) => {
  try {
    ...
    const updated = await cmsService.updatePage(req.params.id, { contentJson });
