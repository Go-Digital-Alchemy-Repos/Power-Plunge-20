...
export default function ProductHighlightBlock({
  ...
  if (loading) {
    return (
      <section
        className={cn("max-w-7xl mx-auto px-4 py-16", settings?.className)}
        data-testid="block-producthighlight"
      >
        <div className="animate-pulse bg-slate-800 rounded-lg h-96" />
      </section>
    );
  }
  ...
  return (
    <section
      className={cn(
        "max-w-7xl mx-auto px-4 py-16 bg-card/30 border-y border-border",
        settings?.className
      )}
      data-testid="block-producthighlight"
    >
      <div className="grid lg:grid-cols-2 gap-12 items-start">
        {showGallery && gallery.length > 0 && (
          <motion.div
            ...
            className="space-y-4"
          >
            <div className="aspect-square bg-black/50 rounded-3xl overflow-hidden border border-border p-4 relative">
              <img
