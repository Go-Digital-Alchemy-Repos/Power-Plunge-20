...
const STORAGE_KEY = "cms_v2_preset_preview";
...
export async function fetchPreview(presetId: string): Promise<{
  ...
  const res = await fetch(`/api/admin/cms-v2/site-presets/${presetId}/preview`, {
    method: "POST",
...
export async function activatePreset(presetId: string): Promise<{
  ...
  const res = await fetch(`/api/admin/cms-v2/site-presets/${presetId}/activate`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
  });
...
export async function rollbackPreset(): Promise<{
  ...
  const res = await fetch("/api/admin/cms-v2/site-presets/rollback", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
  });
