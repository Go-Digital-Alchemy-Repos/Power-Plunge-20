...
```
POST /api/admin/cms-v2/site-presets/:id/preview
```
...
```
POST /api/admin/cms-v2/site-presets/:id/activate
```
...
Seeds are loaded via `POST /api/admin/cms-v2/site-presets/seed`. Seeding is idempotent — it checks for existing presets by name before inserting.

## Feature Flag

Site Presets require `CMS_V2_ENABLED=true`. All routes are mounted under `/api/admin/cms-v2/site-presets/` and require admin authentication.
...
- `GET /api/admin/cms-v2/site-settings` — Read current global settings
- `PUT /api/admin/cms-v2/site-settings` — Update global settings directly (independent of presets)
...
| `server/src/schemas/cmsV2.sitePresets.schema.ts` | Zod validation schemas |
| `server/src/repositories/cmsV2.sitePresets.repo.ts` | Database operations (CRUD, snapshots, settings) |
| `server/src/services/cmsV2.sitePresets.service.ts` | Business logic for CRUD and import/export |
| `server/src/services/cmsV2.sitePresets.apply.service.ts` | Preview, activate, and rollback engine |
| `server/src/routes/cmsV2.sitePresets.routes.ts` | API route definitions |
| `server/src/services/cmsV2.siteSettings.service.ts` | Canonical site settings service |
| `server/src/routes/cmsV2.siteSettings.routes.ts` | Site settings GET/PUT endpoints |
| `server/src/data/sitePresetSeeds.ts` | 6 pre-configured seed presets |
| `client/src/pages/admin-cms-v2-presets.tsx` | Presets Manager admin UI |
