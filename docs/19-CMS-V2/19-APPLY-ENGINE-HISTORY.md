...
```
client/src/admin/cms-v2/presets/applyPreset.client.ts  → Client-side preview helpers
server/src/services/cmsV2.sitePresets.apply.service.ts → Apply engine (preview, activate, rollback)
server/src/repositories/cmsV2.sitePresets.repo.ts      → Snapshot storage, site settings writes
shared/schema.ts                                       → presetApplyHistory table definition
```

## Apply Engine Service

The `SitePresetApplyService` class (`server/src/services/cmsV2.sitePresets.apply.service.ts`) exposes three core operations:
...
|--------|----------|---------|
| `GET` | `/api/admin/cms-v2/site-presets/apply-history` | List all activation history entries (newest first) |
| `GET` | `/api/admin/cms-v2/site-presets/apply-history/:id` | Get a single history entry with full snapshot |
...
1. Navigate to `/admin/cms-v2/presets`
...
```
POST /api/admin/cms-v2/site-presets/rollback
```
...
1. **Verify preview:** `POST /api/admin/cms-v2/site-presets/:id/preview` should return a diff without modifying any data
2. **Verify activation:** `POST /api/admin/cms-v2/site-presets/:id/activate` should create a history entry and update `site_settings`
3. **Verify snapshot:** `GET /api/admin/cms-v2/site-presets/apply-history` should show the activation with full snapshot data
4. **Verify rollback:** `POST /api/admin/cms-v2/site-presets/rollback` should restore the previous settings and mark the snapshot as rolled back
...
|------|---------|
| `server/src/services/cmsV2.sitePresets.apply.service.ts` | Apply engine (preview, activate, rollback) |
| `server/src/repositories/cmsV2.sitePresets.repo.ts` | Snapshot CRUD, settings read/write |
| `client/src/admin/cms-v2/presets/applyPreset.client.ts` | Client-side preview/activate/rollback helpers |
