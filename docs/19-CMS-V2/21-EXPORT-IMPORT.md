...
```
GET /api/admin/cms-v2/site-presets/:id/export
```
...
```
POST /api/admin/cms-v2/site-presets/import
...
- To ensure kits are available, navigate to `/admin/cms-v2/sections` and click "Load Starter Kits" in the target environment
...
| File | Purpose |
|------|---------|
| `server/src/services/cmsV2.sitePresets.service.ts` | `exportPreset()` and `importPreset()` methods |
| `server/src/routes/cmsV2.sitePresets.routes.ts` | `GET /:id/export` and `POST /import` endpoints |
| `server/src/schemas/cmsV2.sitePresets.schema.ts` | Validation schema for import data |
| `client/src/pages/admin-cms-v2-presets.tsx` | Export button and import file handler in UI |
