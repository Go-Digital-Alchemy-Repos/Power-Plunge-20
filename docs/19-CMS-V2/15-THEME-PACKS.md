...
The CMS v2 has two theming systems that work together:
...
Clicking a pack card in `/admin/cms-v2/themes` (Theme Packs tab) instantly applies the pack's combined CSS variables to the admin UI. This is a DOM-only change — no data is persisted.
...
1. Sends `POST /api/admin/cms-v2/theme-packs/activate` with the pack ID
...
The themes page (`/admin/cms-v2/themes`) uses an underline-style tab interface:
...
|--------|----------|------|---------|
| `GET` | `/api/admin/cms-v2/theme-packs` | Admin | List all 5 theme pack presets |
| `GET` | `/api/admin/cms-v2/theme-packs/active` | Admin | Get the currently active pack (or null) |
| `POST` | `/api/admin/cms-v2/theme-packs/activate` | Admin | Activate a pack by ID |
| `POST` | `/api/admin/cms-v2/theme-packs/deactivate` | Admin | Deactivate the current pack (reverts to color theme) |
...
| `client/src/pages/admin-cms-v2-themes.tsx` | Admin UI with preview/activate |
| `client/src/components/ThemeProvider.tsx` | Public page theme application |
| `server/src/routes/admin/cms-v2.router.ts` | API endpoints for pack management |
