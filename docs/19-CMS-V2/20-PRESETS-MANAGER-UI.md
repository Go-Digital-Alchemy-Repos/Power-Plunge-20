...
The Presets Manager is the admin interface for creating, editing, previewing, activating, and managing Site Presets. It is accessible at `/admin/cms-v2/presets` and appears in the CMS v2 sidebar under "Presets."

## Access

- **Admin URL:** `/admin/cms-v2/presets`
- **Sidebar:** CMS v2 → Presets
...
1. **Navigate to Presets page:** Go to `/admin/cms-v2/presets` — the page loads with preset cards (or an empty state with the Seed Presets button)
...
| File | Purpose |
|------|---------|
| `client/src/pages/admin-cms-v2-presets.tsx` | Full Presets Manager page component |
| `client/src/admin/cms-v2/presets/applyPreset.client.ts` | Client-side preview/activate/rollback API calls |
| `client/src/cms/presets/sitePresets.types.ts` | TypeScript interfaces |
| `server/src/routes/cmsV2.sitePresets.routes.ts` | API routes |
