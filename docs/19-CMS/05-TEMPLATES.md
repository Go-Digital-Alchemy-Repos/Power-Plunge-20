...
1. Navigate to **CMS → Templates** to browse all available templates
...
3. The template automatically appears in:
   - The Templates page (`/admin/cms/templates`)
...
- **Set Home:** `POST /api/admin/cms/pages/:id/set-home` clears `isHome` on all other pages, then sets it on the target
- **Set Shop:** `POST /api/admin/cms/pages/:id/set-shop` clears `isShop` on all other pages, then sets it on the target

Both operations use database transactions to prevent race conditions.

## Template Preview System

The Templates page (`/admin/cms/templates`) displays each template as a visual card with:
