...
CMS provides a non-destructive migration path for converting legacy HTML pages into the block-based content system.

## Overview

Legacy CMS pages store their content as raw HTML in the `content` text column. CMS pages store structured block data in the `contentJson` JSONB column. The migration tool converts the former into the latter without destroying any data.
...
```
POST /api/admin/cms/pages/:id/migrate-to-blocks
```
...
In the CMS pages list (`/admin/cms/pages`), each page's action menu conditionally shows a **"Migrate to Blocks"** option when:
