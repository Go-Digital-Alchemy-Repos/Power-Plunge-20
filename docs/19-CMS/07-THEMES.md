...
The CMS theme system uses a structured token schema to control the visual appearance of the storefront. Tokens are organized into seven groups (colors, typography, radius, shadows, spacing, buttons, layout) and are applied to the DOM as CSS custom properties prefixed with `--pp-`.
...
server/src/
  routes/admin/cms.router.ts   → Theme API endpoints
  services/cms.service.ts      → Theme activation logic
```

### Data Flow

1. Admin selects a theme preset in `/admin/cms/themes`
2. Preview mode: `applyThemeTokens(tokens, "preview")` applies variables to `[data-pp-theme-scope]` element
3. Activate: `POST /api/admin/cms/themes/activate` stores the preset ID in `site_settings.active_theme_id`
...
- **Use case:** Admin clicks a preset card in `/admin/cms/themes` to see how it looks before committing.

### Activate

- **Function:** `POST /api/admin/cms/themes/activate` with `{ themeId: "preset-id" }`
...
Or via API:
```
POST /api/admin/cms/themes/activate
{ "themeId": "ocean-blue" }
```
...
|--------|----------|------|---------|
| `GET` | `/api/admin/cms/themes` | Admin | List all 10 theme presets with full token data |
| `GET` | `/api/admin/cms/themes/active` | Admin | Get the currently active theme preset |
| `GET` | `/api/theme/active` | Public | Get active theme (used by ThemeProvider on public pages) |
| `POST` | `/api/admin/cms/themes/activate` | Admin | Activate a theme by preset ID |
